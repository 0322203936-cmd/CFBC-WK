...
window.filterProdRows = function() {
  var term = (document.getElementById('prodFilter')||{}).value;
  term = term ? term.toLowerCase() : '';
  var rows = document.querySelectorAll('.pt-prod-row');
  var nuevoTotal = 0;
  for(var i=0; i<rows.length; i++){
  ...
  var metaSpan = document.getElementById('gastoTotalVal');
  if (metaSpan) {
...
function showProdPanel(rowData, opts) {
  ...
  if (hideProducts && !siembraRowsHtml) {
    ...
    var panelMeta = hideProducts
      ...
      : ('Gasto: <b style="color:#16a34a" id="gastoTotalVal">'+_fmtG(total)+'</b>' + (_isUSD ? ' <span style="font-size:9px;color:#94a3b8;">USD @$'+_TC+'</span>' : ''));
      
    var searchHtml = hideProducts ? '' : '<input type="text" id="prodFilter" placeholder="Buscar producto..." style="padding:3px 8px; border:1px solid #cbd5e1; border-radius:4px; font-size:11px; width:180px; margin-left:20px" onkeyup="filterProdRows()">';
