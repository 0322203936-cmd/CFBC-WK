...
function renderRancho() {
  ...
  function sumForYW(records, yr, wk){
    ...
    records.forEach(function(r){
      ...
      Object.keys(ranches||{}).forEach(function(rn){
        if(out[rn]!==undefined) out[rn]+=(ranches[rn]||0);
      });
    });
    return out;
  }
  ...
  h1+='<th colspan="'+nCols+'" style="'+thB+'text-align:center;border-left:3px solid #4472C4;background:#9DC3E6">TOTAL '+sym+'</th>';
  h1+='</tr>';
  ...
  CATS.forEach(function(c,ci){
    ...
    RANCH_ORDER.forEach(function(rn){ bodyHtml+=groupCells(rn); });
    bodyHtml+=groupCells(null); // TOTAL
    bodyHtml+='</tr>';
  });

  // ── Leer costos unitarios directo de DATA.unit_cost_data ─────────────────
  function buildUcData(section){
    // Returns ywUcData[yr][wk] = {mat,mo,sv,cpv}  each = {total, ranches:{rn:val}}
    var ucd=DATA.unit_cost_data||[];
    var res={};
    yrs.forEach(function(yr){
      res[yr]={};
      rangeWeeks.forEach(function(wk){
        res[yr][wk]={mat:{total:0,ranches:{}},mo:{total:0,ranches:{}},sv:{total:0,ranches:{}},cpv:{total:0,ranches:{}}};
        RANCH_ORDER.forEach(function(rn){
          ['mat','mo','sv','cpv'].forEach(function(f){ res[yr][wk][f].ranches[rn]=0; });
        });
      });
    });
    ucd.forEach(function(r){
      if(r.section!==section) return;
      if(!res[r.year]||!res[r.year][r.week]) return;
      var slot=res[r.year][r.week][r.field];
      if(!slot) return;
      slot.total=Math.round((slot.total+(r.mxn_total||0))*100)/100;
      Object.keys(r.mxn_ranches||{}).forEach(function(rn){
        if(slot.ranches[rn]!==undefined)
          slot.ranches[rn]=Math.round((slot.ranches[rn]+(r.mxn_ranches[rn]||0))*100)/100;
      });
    });
    return res;
  }

  // ── Construir tabla de costos unitarios ───────────────────────────────────
  function buildUnitTable(titulo, section, extraRows){
    var ywUc=buildUcData(section);
    ...
    function groupCellsUnit(rnKey, costKey){
      ...
      yrs.forEach(function(yr,yi){
        var col=YEAR_COLORS[yr]||'#888';
        rangeWeeks.forEach(function(wk,wi){
          var d=ywUc[yr][wk][costKey];
          var v=rnKey!==null?(d.ranches[rnKey]||0):(d.total||0);
          var lb=wi===0?'2px solid '+col:'';
          s+=cell(v, false, costKey==='cpv', lb);
        });
        if(nWk>=2){
          var fd=ywUc[yr][rangeWeeks[0]][costKey];
          var ld=ywUc[yr][rangeWeeks[nWk-1]][costKey];
          var fv=rnKey!==null?(fd.ranches[rnKey]||0):(fd.total||0);
          var lv=rnKey!==null?(ld.ranches[rnKey]||0):(ld.total||0);
          s+=cell(Math.round((lv-fv)*100)/100, true, costKey==='cpv', '1px solid #aaa');
        }
      });
      if(nYrs>=2){
        var d0=ywUc[yrs[0]][rangeWeeks[nWk-1]][costKey];
        var dn=ywUc[yrs[nYrs-1]][rangeWeeks[nWk-1]][costKey];
    ...
    (extraRows||[]).forEach(function(r,ri){
      ...
      t+='<tr style="background:'+bg+'"><td style="padding:3px 8px;position:sticky;left:0;z-index:1;background:'+bg+';border-bottom:1px solid #eee;border-right:1px solid #ddd;white-space:nowrap;color:#555;font-weight:500">'+r+'</td>';
